...
class NodeFactory:
    ...
    def _register_builtin_nodes(self) -> None:
        """내장 노드 타입들 등록"""
        self.register("GilConnectorOpenAI", GilConnectorOpenAI)
        self.register("GilGenImage", GilGenImage)
    ...
    def create_node(self, node_type: str, config: Dict[str, Any], name: str = None) -> GilNode:
        """노드 인스턴스 생성"""
        ...
        elif node_type == "GilGenImage":
            return self._create_image_generator(node_class, config, name)
        else:
            # 기본 생성
            return node_class(name=name or f"{node_type}_instance", **config)
    
    def _create_openai_connector(self, node_class: Type, config: Dict[str, Any], name: str) -> GilNode:
        """OpenAI 커넥터 생성"""
        # 환경변수에서 API 키 가져오기
        import os
        ...
        return node_class(
            name=name or "OpenAI Connector",
            api_key=api_key,
            organization=config.get("organization"),
            base_url=config.get("base_url")
        )
    
    def _create_image_generator(self, node_class: Type, config: Dict[str, Any], name: str) -> GilNode:
        """이미지 생성기 생성"""
        ...
        dummy_connector = GilConnectorOpenAI(api_key=api_key)
        
        return node_class(
            name=name or "Image Generator",
    ...
    def get_node_info(self, node_type: str) -> Dict[str, Any]:
        ...
        try:
            if node_type == "GilConnectorOpenAI":
                dummy_instance = node_class(api_key="dummy")
            elif node_type == "GilGenImage":
                dummy_connector = GilConnectorOpenAI(api_key="dummy")
                dummy_instance = node_class(connector=dummy_connector)
            else:
                dummy_instance = node_class(name="dummy")
